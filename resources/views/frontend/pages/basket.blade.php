            ...
            <div class="row mb-5">
                    ...
                    <div class="col-lg-12 site-blocks-table">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($cardItem as $key => $card)

                                    <tr>
                                        <td class="product-thumbnail">
                                            <img src="{{asset($card['image'])}}" alt="Image" class="img-fluid">
                                        </td>
                                        <td class="product-name">
                                            <h2 class="h5 text-black">{{$card['name'] ?? ''}}</h2>
                                        </td>
                                        <td>${{$card['price']}}</td>
                                        <td>
                                            <div class="input-group mb-3" style="max-width: 120px;">
                                                ...
                                                <input type="text" class="form-control text-center" value="{{$card['qty']}}"
                                        ...
                                        <td>${{$card['price']*$card['qty']}}</td>
                                        <td>
                                            <form action="{{route('sepet.remove')}}" method='POST'>
                                                @csrf
                                                <input type="hidden" name="product_id" value="{{$key}}">
                                                <button type="submit" href="#" class="btn btn-primary btn-sm">Sil</button>
                                            </form>
                                        </td>
                                    </tr>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <button class="btn btn-primary btn-lg py-3 btn-block"
                                        onclick="window.location='checkout.html'">Ödemeyi Bitir</button>
                                </div>
                            </div>
                        </div>
                    </div>
